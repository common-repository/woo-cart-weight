...
class StoreEndpoint implements Hookable
{
    ...
    protected function prepare_checkout_endpoint_data(): array
    {
        ...
        $data['endpoint'] = CheckoutSchema::IDENTIFIER;
        return $data;
    }
    protected function prepare_cart_endpoint_data(): array
    {
        $data = $this->prepare_endpoint_data();
        $data['endpoint'] = CartSchema::IDENTIFIER;
        return $data;
    }
    ...
    {
        return apply_filters('octolize-checkout-block-integration-' . $this->integration_name . '-schema', [$this->field_name => ['description' => __('Field', 'woo-cart-weight'), 'type' => ['string'], 'context' => ['view', 'edit'], 'readonly' => \false, 'optional' => \false, 'arg_options' => ['validate_callback' => function ($value) {
            return \true;
        }]]]);
    }
}
